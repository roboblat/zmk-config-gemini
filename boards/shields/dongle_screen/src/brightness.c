...
/* Position events fire on the central for every keypress from any peripheral,
   upstream of keycode processing. On ZMK main/Zephyr 4.1 the keycode wake path
   is unreliable on dongle centrals (see zmk #3195/#3207), so we ALSO wake on
   position events as a robust second path. */
#include <zmk/events/position_state_changed.h>
/* Activity-state events fire ACTIVE on any input, including the relayed
   trackpad (ZMK's activity input listener catches all input devices). Used to
   wake the screen on trackpad movement, not just key presses. */
...
{
    /* Trackpad (and any input) wake: activity goes ACTIVE on pointer motion. */
    ...
    if (act)
    {
        ...
        {
#if CONFIG_DONGLE_SCREEN_IDLE_TIMEOUT_S > 0
            LOG_DBG("Wake via activity ACTIVE (e.g. trackpad)");
            last_activity = k_uptime_get();
...
#if CONFIG_DONGLE_SCREEN_IDLE_TIMEOUT_S > 0
    // Reached by keycode events (non-hotkey) AND by position_state_changed
    // events (ev == NULL for those). Position events are the reliable wake path
    // on dongle centrals where the keycode path is flaky on Zephyr 4.1.
    if (!ev)
    {
        LOG_DBG("Wake/activity via position event (peripheral keypress)");
    }
    last_activity = k_uptime_get();
    if (!screen_on && !off_through_modifier)
    {
...
/* Robust wake path for dongle centrals: every peripheral keypress raises a
   position event on the central, even when the keycode wake path misbehaves
   on Zephyr 4.1. */
ZMK_SUBSCRIPTION(screen_idle, zmk_position_state_changed);
/* Trackpad / pointer wake: activity goes ACTIVE on any input event. */
